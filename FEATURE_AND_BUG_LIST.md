...
- [x] **应用图标重新裁切 (App Icon Recrop)**
  - 原图标是「方块套方块」：裁切保留了原稿画布的外框与圆角描边，装到桌面上再被系统圆角一裁，就成了两层。
  - 现在裁在圆角描边**以内**——只裁到直边内侧还不够，四角圆弧仍会露出来，必须缩进到圆角半径之内——只留霓虹粉光渗到边缘的悬浮感。iOS / Android（含圆形图标）/ macOS / Web 全套 37 张重出。
