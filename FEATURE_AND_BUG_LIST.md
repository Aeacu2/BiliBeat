...
- [x] **玻璃只留给「卡片」，歌曲是「行」不是「盒子」(Glass for Cards, Rows for Songs)**
  ...
  - 玻璃保留给真正是卡片的东西：搜索框、快捷入口（已下载 / 收藏）、歌单卡片、背景。
...
- [x] **歌单改为竖排条形卡 (Playlists as a Vertical Stack)**
  - 原来是横向滑动的方卡：第二张之后的歌单全藏在一个没人想得到去试的横向滚动里，而页面本身是纵向滚动的。现在是通栏条形卡自上而下排列，末尾一条虚线框的「新建歌单」。
  - 最近播放**仍是横向滚动**：它是「最近」的时间流，天然有限且可无限增长，不该把首页撑长。
- [x] **品牌资产由脚本从原稿重建 (Brand Assets, Rebuilt From Source)**
  ...
  - **抠图按「色度」而非亮度**：`R - max(G, B)`。原稿的方块、圆角描边与画布都是中性甚至偏蓝（描边采样为 80,79,85），色度恰好为零；而霓虹与它的辉光色度全为正。按亮度抠是不行的——描边比辉光外圈还亮，会活下来，在标识后面留下一个方框。笔画最亮的核心接近白色，靠亮度补救，但**必须限定在仍有粉色的地方**，否则方块下面那行白色 wordmark 也会被一起抠出来。
  - **裁切范围必须留在方块以内**：原稿的辉光在方块边界处是被**裁断**的（像素里真实存在的一道台阶），任何颜色抠图都消不掉；框内一旦包含那条边界，标识背后就会出现一个圆角方形。所以裁在边界以内，再对画面最外圈做羽化，否则辉光会在半路被切断，同样留下一圈方框。
  - **图标是重建的，不是裁出来的**：深色底板（中心微亮的径向渐变）＋ 抠出的霓虹标识居中合成，辉光自然渗进黑底，四周留出黑边。这样既没有「方块套方块」的两层感，也保留了黑色里透出的粉光。
...
- [x] **播放卡与播放页融为一体 (Docked Card ⇄ Player, One Object)**
  ...
  - **主控圆钮不沿用播放页的粉色实心**：连续性靠的是形状与位置，饱和的粉色圆盘缩到 68pt 的条上会变成整屏最吵的东西，还要跟紧邻的封面抢注意力。改为与玻璃表面同一套顶部高光 + 发丝描边的半透明圆钮，粉色只留给底边进度线。
...
- [x] **拖拽歌词校准时间轴 (Drag-to-Calibrate Lyrics)**
  ...
  - **取代了原来的 ±0.1s / ±0.5s 按钮**。盲点微调本就是错的交互：零点一秒是看不出来的，只能点一下听一下、再点一下再听一下，撞运气撞到对齐为止。现在长按预览、把「此刻该唱的那句」拖到中线松手——拖动这个动作本身就是那个修正量。
  - 拖动时实时显示「延迟 / 提前 X.X 秒」，且**随播放进度动态更新**（基准是活的播放位置）；编辑的不是当前播放曲目时基准为 0:00（此时编辑器拿到的是空位置时钟，而不是另一首歌的进度）。
  - 普通滑动仍然是滚动歌词，两者由手势竞技场区分（按住不动 → 校准；直接滑 → 滚动）。
  - 偏移在点「应用」时随歌词一起写入（沿用既有的 `_applyLyricResult`），不需要另一条持久化路径。
